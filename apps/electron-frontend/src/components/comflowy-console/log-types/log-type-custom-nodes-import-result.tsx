...
export function LogTypeCustomNodesImportResult({log}: {log: ComflowyConsoleLog}) {
  ...
  return (
    <Log log={log} level={importFailedExtensions.length > 0 ? "warn" : "info"} title={`Load custom nodes ${importSuccessExtensions.length} success, ${importSuccessExtensions.length} failed`} className={`log-type-custom-nodes-import-result`}>
